...
for wiersz in wiersze:
    ...
    for dzien in range(5):
        ...
        if wszystkie_przedmioty:
            lekcja = wszystkie_przedmioty[wybrany_nr].get_text(strip=True)
        else:
            lekcja = "brak"

        nauczyciele_html = komorka.find_all("a", class_="n")
        ...
        if lekcja.endswith("-2/2"):
            lekcja = lekcja[:-4].strip()
